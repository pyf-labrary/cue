...
/**
 * Stripped variant for legacy scenePlayer (Scenes detail page).
 * Only space / arrows / home / escape — no lane keys, since legacy keeps
 * mute/solo inside scenePlayer state, not in a composition.
 */
export function useLegacyTransportShortcuts({
  enabled = true,
  player,
}: {
  enabled?: boolean;
  player: {
    getState: () => { status: string; currentSec: number };
    play: () => void;
    pause: () => void;
    stop: () => void;
  ...
  useEffect(() => {
    ...
    function handler(e: KeyboardEvent) {
      ...
      switch (e.key) {
        case ' ':
          e.preventDefault();
          if (s.status === 'playing') player.pause();
          else player.play();
          break;
        case 'ArrowLeft': {
          e.preventDefault();
          const step = e.shiftKey ? 5 : 2;
          player.seek(Math.max(0, s.currentSec - step));
          break;
        }
        case 'ArrowRight': {
          e.preventDefault();
          const step = e.shiftKey ? 5 : 2;
          player.seek(s.currentSec + step);
          break;
        }
        case 'Home':
        case '0':
